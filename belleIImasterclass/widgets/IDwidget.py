...
class IDWidget:
    def __init__(self,particles_manager: ParticlesManager,cheat_mode=True, cheating_threshhold = 1e-2) -> None:
        self.pm=particles_manager
        self.cheat_mode=cheat_mode
        self.make_true_particle_data()

        self.energies=self.pm._df.loc[:,"ecl_energy"].to_numpy()[self.pm.index]
        self.px = self.pm._df.loc[:,"tracker_pt"].to_numpy()*np.cos(self.pm._df.loc[:,"tracker_phi"].to_numpy())[self.pm.index]
        self.py = self.pm._df.loc[:,"tracker_pt"].to_numpy()*np.sin(self.pm._df.loc[:,"tracker_phi"].to_numpy())[self.pm.index]
        self.pz = self.pm._df.loc[:,"pz"].to_numpy()[self.pm.index]
        self.KLM_hits = self.pm._df.loc[:,"klm_detect"].to_numpy()[self.pm.index]
        self.charge=self.pm._df["tracker_charge"].to_numpy()[self.pm.index]        
        if(self.cheat_mode):
            self.px_cheat_mask = (abs(self.px-self.pm._df["px"].to_numpy())<0.01)
            self.py_cheat_mask = (abs(self.px-self.pm._df["px"].to_numpy())<0.01)
            self.energies_cheat_mask = (abs(self.energies-self.pm._df["energy"].to_numpy())<0.01)
        ...
        for i in range(self.pm.n_particles):
            self.patch_collections.append(PatchCollection(self.pm._df.loc[i,"patches"],color=self.pm._df.loc[i,"colors"]))
        self.impuls = np.sqrt(self.px**2 + self.py**2 + self.pz**2)
        self.masse = np.sqrt(abs(self.energies**2 - self.impuls**2))

    def update_values(self, change = 0): 
        self.energies=self.pm._df.loc[:,"ecl_energy"].to_numpy()[self.pm.index]
        self.px = self.pm._df.loc[:,"tracker_pt"].to_numpy()*np.cos(self.pm._df.loc[:,"tracker_phi"].to_numpy())[self.pm.index]
        self.py = self.pm._df.loc[:,"tracker_pt"].to_numpy()*np.sin(self.pm._df.loc[:,"tracker_phi"].to_numpy())[self.pm.index]
        self.pz = self.pm._df.loc[:,"pz"].to_numpy()[self.pm.index]
        ...
        self.charge=self.pm._df["tracker_charge"].to_numpy()[self.pm.index]        
        if(self.cheat_mode):
            self.px_cheat_mask = ((self.px-self.pm._df["px"].to_numpy())<0.01)
            self.py_cheat_mask = ((self.px-self.pm._df["px"].to_numpy())<0.01)
            self.energies_cheat_mask = ((self.energies-self.pm._df["energy"].to_numpy())<0.01)
            self.px[self.px_cheat_mask] =  self.pm._df.loc[self.px_cheat_mask,"px"].to_numpy()
            self.px[self.px_cheat_mask] =  self.pm._df.loc[self.px_cheat_mask,"py"].to_numpy()
            self.energies[self.energies_cheat_mask] = self.pm._df.loc[self.energies_cheat_mask,"energy"] 
        for i in range(self.pm.n_particles):
    ...
    def show(self):
        ...
        self.update_button.observe(self.update_values, names = "value")
        ...
        for i in range(self.pm.n_particles):
            ...
            self.res_box.layout = widgets.Layout(border='solid 1px black',margin='0px 10px 10px 0px',padding='5px 5px 5px 5px',height = "730px ",width = "370px")            
            ...
            self.sel_box.layout = widgets.Layout(border='solid 1px black',margin='0px 10px 10px 0px',padding='5px 5px 5px 5px',height = "730px ",width = "370px")  
